...
//  }
  ...
  def main(args : Array[String]) = {
    ...
    val elapsed : (=> Unit) => Long = f => {
      val s = System.currentTimeMillis; f; (System.currentTimeMillis - s)/1000
    }

    def timeTaken(callback: () => Long) {
      val s = System.currentTimeMillis; callback; (System.currentTimeMillis - s)/1000
    }
    ...
    pairs2 foreach { pair =>
      ...
      println{ "lcsBruteforceRecursive(\""+pair._1+"\",\""+pair._2+"\") = \""+s+"\"   ("+time+" sec)" }
